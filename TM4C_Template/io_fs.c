...
fs_open(const char *pcName)
{
    ...
    //
    // Read ROM data by index
    //
    else if (ustrncmp(pcName, "/romdata", 8) == 0)
    {
        static char pcBuf[100];
        char *req_line = (char *)pcName + 15;
        uint32_t romIndex = 0;

        // Convert to decimal number
        while ((*req_line >= '0') && (*req_line <= '9'))
        {
            romIndex *= 10;
            romIndex += (*req_line - '0');
            req_line++;
        ...
        // usnprintf(pcBuf, 100, eeprom_read_buffer[10]);
        // int number = (int)strtol(romIndex, NULL, 16);
        // UARTprintf("Read[%d]\n", romIndex);
        sprintf(pcBuf, "{\"MyKey\":%d}", eeprom_read_buffer[romIndex]);
        psFile->data = pcBuf;
    ...
    //
    // Read all data from ROM in JSON format
    //
    else if (ustrncmp(pcName, "/getROM", 7) == 0)
    {
        UARTprintf("Get ROM\n");
        // static char pcBuf[1500];
        // sprintf(pcBuf, "%s", eeprom_create_json());
        psFile->data = eeprom_create_json();
        psFile->len = strlen(eeprom_create_json());
        psFile->index = psFile->len;
        psFile->pextension = NULL;
        return (psFile);
    }
    ...
    // Read all data from ROM in JSON format
    //
    else if (ustrncmp(pcName, "/massEraseROM", 13) == 0)
    {
        uint32_t ret;
        static char pcBuf[30];
        UARTprintf("Mass Erase ROM\n");
        ret = eeprom_mass_erase();
        if (ret == 0) {
            sprintf(pcBuf, "{\"result\":\"%s\"}", "SUCCESS");
        }
        else {
            sprintf(pcBuf, "{\"result\":\"%s\"}", "FAILED");
        }
        psFile->data = pcBuf;
        psFile->len = strlen(pcBuf);
        psFile->index = psFile->len;
